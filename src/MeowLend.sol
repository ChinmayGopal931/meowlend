...
contract MeowLend {
    ...
    function deposit(uint amount) external {
        ...
        lpToken.totalSupply() == 0 
            ? lpAmount = amount 
            : lpAmount = amount * lpToken.totalSupply() / usdc.balanceOf(address(this));
    ...
    function payback(uint amount) external {
        ...
        account.debt -= paybackAmount;
        totalDebt    -= paybackAmount;
    ...
    function calculateInterest(Account storage account) internal view returns (uint) {
        ...
        uint interest    = account.debt * INTEREST_RATE * timeElapsed / 365 days / 100;
        return interest;
    }
